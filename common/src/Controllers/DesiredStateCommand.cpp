...
template
struct DesiredStateData<double>;
template
struct DesiredStateData<float>;
...
void DesiredStateCommand<T>::convertToStateCommands() {
    ...
    if (parameters->use_rc) {
        ...
        if (use_beiTong) { // use BeiTong
            joystickLeft = beiTong->leftStick;
            joystickRight = beiTong->rightStick;
            trigger_pressed = beiTong->a;
        }
    }
    // Warning!!!!
...
template
class DesiredStateCommand<double>;

template
class DesiredStateCommand<float>;
