...
bool JPosInitializer<T>::IsInitialized(LegController<T> *ctrl) {
    ...
    if (_curr_time < _end_time) {
        ...
        for (size_t leg(0); leg < cheetah::num_leg; ++leg) {    // [0, 4)
            for (size_t jidx(0); jidx < cheetah::num_leg_joint; ++jidx) {   // [0, 3)
...
//    (void)ctrl;
    T ini[3 * cheetah::num_act_joint];    // 12
...
void JPosInitializer<T>::_UpdateParam() {
    ...
    handler.getVector<T>("delta_jpos", _target_jpos);
    handler.getVector<T>("mid_jpos", _mid_jpos);
}

template
class JPosInitializer<double>;

template
class JPosInitializer<float>;
