...
void printUsage() {
    printf(
            ...
            "\twhere robot-id:     3 for cheetah 3, m for mini-cheetah, l for leopard\n"
...
int main_helper(int argc, char **argv, RobotController *ctrl) {
    ...
    if (argv[1][0] == '3') {
        ...
        gMasterConfig._robot = RobotType::LEOPARD;
    } else {
        printUsage();
        return EXIT_FAILURE;
    }
    ...
    printf("        Quadruped:  %s\n",
           gMasterConfig._robot == RobotType::MINI_CHEETAH
           ? "Mini Cheetah"
           : (gMasterConfig._robot == RobotType::CHEETAH_3 ? "Cheetah 3" : "Leopard"));
    ...
    if (gMasterConfig.simulated) {
        ...
        if (gMasterConfig._robot == RobotType::MINI_CHEETAH) {
            ...
            LeopardHardwareBridge hw(ctrl, gMasterConfig.load_from_file);
            hw.run();
        } else {
            printf("[ERROR] unknown robot\n");
            assert(false);
