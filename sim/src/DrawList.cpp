...
/*!
 * Load the TJ Leopard model and builds the draw list.
 * Returns an index number that can later be used to update the position of the
 * robot.
 */
size_t DrawList::addTJLeopard(Vec4<float> color, bool useOld, bool canHide) {

    size_t i0 = _tjLeopardLoadIndex;  // todo don't hard code this
    size_t j0 = _nTotal;
    ...
    SolidColor bodyColor, abadColor, link1Color, link2Color;
    bodyColor.rgba = useOld ? Vec4<float>(.7, .0, .0, .9) : color;
    bodyColor.useSolidColor = true;

    abadColor.rgba = useOld ? Vec4<float>(.0, .7, .0, .9) : color;
    abadColor.useSolidColor = true;

    link1Color.rgba = useOld ? Vec4<float>(.7, .7, .0, .9) : color;
    link1Color.useSolidColor = true;

    link2Color.rgba = useOld ? Vec4<float>(.1, .0, .7, .9) : color;
    link2Color.useSolidColor = true;
