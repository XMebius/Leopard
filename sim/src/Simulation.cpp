...
Simulation::Simulation(RobotType robot, Graphics3D *window,
    ...
    _robot = robot;
    _quadruped = (_robot == RobotType::MINI_CHEETAH || _robot == RobotType::LEOPARD) ? buildMiniCheetah<double>() :
    ...
    if (_window) {
        ...
        _simRobotID = _robot == RobotType::MINI_CHEETAH ? window->setupMiniCheetah(truthColor, true, true) :
                      _robot == RobotType::LEOPARD ? window->setupTJLeopard(truthColor, true, true) :
                      window->setupCheetah3(truthColor, true, true);

        // 若使用cheaterMode则没有这个
        _controllerRobotID = _robot == RobotType::MINI_CHEETAH ? window->setupMiniCheetah(seColor, false, false) :
                             _robot == RobotType::LEOPARD ? window->setupTJLeopard(seColor, false, false) :
    ...
    // init spine:
    if (_robot == RobotType::MINI_CHEETAH || _robot == RobotType::LEOPARD) {
    ...
    } else if (_robot == RobotType::LEOPARD) {
        _robotParams.initializeFromYamlFile(getConfigDirectoryPath() +
                                            LEOPARD_DEFAULT_PARAMETERS);
    } else {
        assert(false);
    }
...
void Simulation::sendControlParameter(const std::string &name,
    ...
    // message data
    request.requestKind = isUser ? ControlParameterRequestKind::SET_USER_PARAM_BY_NAME
